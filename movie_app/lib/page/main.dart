...
import 'package:movieapp/repository/movie_repositoryImpl.dart';
import 'movieDatail_page.dart';
...
class _MyMovieApp extends State<MyMovieApp> {
  ...
  Future<Movie> _fetchUpcomingMovies() async {
    final response = await http.get(Tmdb.upcomingUrl);
  ...
  Future<Movie> _fetchPopularMovies() async {
    final response = await http.get(Tmdb.popularUrl);
    if (response.statusCode == 200) {
      var decodedJson = jsonDecode(response.body);
      setState(() {
  ...
  Future<Movie> _fetchTopRatedMovies() async {
    final response = await http.get(Tmdb.topRatedUrl);
    if (response.statusCode == 200) {
      var decodedJson = jsonDecode(response.body);
      setState(() {
